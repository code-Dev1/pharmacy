        ...
        <x-card class="xl:col-span-2" :title="__('reports.sales_report')" :description="__('dashboard.today_sales_total')">
            ...
            <div class="flex h-72 items-end gap-3 rounded-3xl bg-gradient-to-b from-slate-50 to-white p-4 dark:from-white/[0.05] dark:to-transparent">
                @foreach ($salesTrend as $point)
                    <div class="flex flex-col justify-end flex-1 h-full gap-3">
                        <div class="relative flex items-end flex-1 group">
                            <div
                                class="w-full transition shadow-lg rounded-t-2xl bg-gradient-to-t from-emerald-600 via-teal-400 to-sky-300 shadow-emerald-900/10 group-hover:from-emerald-500"
                                style="height: {{ max(($point['sales'] / $max) * 100, 4) }}%"
                            ></div>
                        </div>
                        <span class="text-xs font-medium text-center truncate text-slate-500 dark:text-slate-400">{{ $point['date'] }}</span>
                    </div>
                @endforeach
            </div>
        </x-card>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-white/10">
                        @forelse ($rows as $row)
                            <tr class="ui-row">
                                ...
                                <td class="px-4 py-3 font-semibold text-end text-slate-950 dark:text-white">{{ number_format($row->total, 2) }}</td>
                            </tr>
                        @empty
        ...
        <x-card :title="__('dashboard.low_stock_products')" :description="__('sidebar.low_stock')">
            <div class="space-y-3">
                @forelse ($lowStockProducts as $product)
                    <div class="flex items-center justify-between gap-3 px-4 py-3 text-sm border rounded-2xl border-amber-200/70 bg-amber-50/80 dark:border-amber-400/20 dark:bg-amber-400/10">
        ...
        <x-card :title="__('dashboard.expiring_products')" :description="__('sidebar.expiring_products')">
            <div class="space-y-3">
                @forelse ($expiringProducts as $batch)
                    <div class="flex items-center justify-between gap-3 px-4 py-3 text-sm border rounded-2xl border-rose-200/70 bg-rose-50/70 dark:border-rose-400/20 dark:bg-rose-400/10">
